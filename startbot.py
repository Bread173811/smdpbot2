...
#    print('{}, {} {}: {}'.format (rekt, author, said, content))

@client.event
async def on_guild_join(member):
    await member.add_roles(member, guild.get_role(523005519955230720))
...
async def verify(ctx, *, arg):
    if ctx.channel.id == 521645091098722305:
            ...
            await ctx.message.author.add_roles(ctx.guild.get_role(521645468779020288))
...
class Music:
    ...
    @client.command(name='join', invoke_without_command=True)
    ...
    @client.command(name='now', aliases=['playing', 'current'])
